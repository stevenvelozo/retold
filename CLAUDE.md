...
- **Spawn-target deps:** the docserve and content-editor supervisors don't `require()` their target packages — they `child_process.spawn('node', [<resolved-CLI-path>, ...])`. So a plain source-grep for `require('pict-docuserve')` finds zero hits and looks like the dep is unused, but it isn't. Today the spawn targets are `pict-docuserve` and `retold-content-system`; both are listed in the umbrella `dependencies` block specifically so that `require.resolve('pict-docuserve/source/cli/Docuserve-CLI-Run.js')` (and the equivalent for retold-content-system) works on a fresh clone with only `npm install` run. Each supervisor's `_resolveCliPath()` falls back to the in-monorepo checkout (`modules/pict/pict-docuserve/...`) for in-repo dev workflows, and throws a friendly "X is not installed and the monorepo checkout at … does not exist. Run `npm install` at the retold repo root." error when neither is found, which the API route turns into a 500. **Never remove a dep that's only referenced via `child_process.spawn` or `npx <name>` from the manager — it won't show up in any `require()` audit but the manager really does need it at runtime.**
