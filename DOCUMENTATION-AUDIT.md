...
**Tier 4 — cleanup of existing docs:** — ✅ **done 2026-05-31**: `tidings` README (removed REWRITE/TODO markers, fixed the GraphViz `npm install graphviz` step — `38e0cc7`); `quackage` README (commands aligned to the real CLI: `qua`/`check-dependencies`/`updatepackage` — `acdc240`); `cumulation` README (broken code fence repaired earlier — `27bec5e`). Also: `pict-section-excalidraw` gained a regression test for the `blobReply` fix + request-timeout cleanup (`17e6ee2`). **Remaining:** repoint `retold-data-mapper`'s published site (its `docs/` holds planning artifacts, not user docs). _(Earlier: 3 manifest URLs + `choreographic` entry.)_
