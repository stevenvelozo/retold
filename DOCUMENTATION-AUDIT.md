...
**Tier 3 — pict/utility stubs & thin partials:** pict-section-usermanagement, pict-provider-vocabulary, pict-section-entitymanagement, pict-section-excalidraw, pict-meadow-connection-manager, pict-provider-theme, pict-nonlinearconfig, pict-editor-timeline, cumulation, pict-cruisecontrol, choreographic, elucidator, plus README-only-no-site (pict-renderer-graph, pict-section-theme, fable-ultravisor-client, retold-sharp). — ✅ **all 18 authored + sites built 2026-05-30** (also covered meadow-connection-retold-databeacon and meadow-migrationmanager). Several are scaffolds, documented honestly; the doc pass surfaced **8 real source bugs** — see `codeIssuesFound` in [DOCUMENTATION-GAPS.json](DOCUMENTATION-GAPS.json). The `stricture` README→docs-pages mirror is now done — added `docs/architecture.md` + `docs/quickstart.md` (`cfcf5a6`). _(Minor: the stricture README still says "12 service types"; source registers 13 — left as-is.)_

**Tier 4 — cleanup of existing docs:** — ✅ **done 2026-05-31**: `tidings` README (removed REWRITE/TODO markers, fixed the GraphViz `npm install graphviz` step — `38e0cc7`); `quackage` README (commands aligned to the real CLI: `qua`/`check-dependencies`/`updatepackage` — `acdc240`); `cumulation` README (broken code fence repaired earlier — `27bec5e`). Also: `pict-section-excalidraw` gained a regression test for the `blobReply` fix + request-timeout cleanup (`17e6ee2`). **Also ✅ done:** `retold-data-mapper` — documented the actual shipped **beacon-server** (serve/init + `/mapper/*` REST API + beacon capabilities + MappingConfiguration format) from bin/source, moved the 3 planning artifacts to `planning/`, built site (`e333193`). ⚠️ Found mid-refactor — its README + `examples/bookstore-sync.json` + integration test/harness still describe a **removed** config-sync CLI; recorded in [DOCUMENTATION-GAPS.json](DOCUMENTATION-GAPS.json) (`retold-data-mapper` → `followups`) for separate fixup. _(Earlier: 3 manifest URLs + `choreographic` entry.)_
