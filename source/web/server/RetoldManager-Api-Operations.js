...
module.exports = function registerOperationsRoutes(pCore)
{
	...
	//  body: { Action: 'clean' | 'verify' }
	//  Operates on the global npm cache (not a per-module action), so
	//  this lives under /system/ rather than /all/ or /modules/.
	//  Useful when a publish run leaves the local cache in a bad
	//  state — happens almost daily during heavy ripple work.
	// ─────────────────────────────────────────────
	tmpOrator.serviceServer.doPost('/api/manager/system/operations/npm-cache',
		function (pReq, pRes, pNext)
		{
			let tmpBody = pReq.body || {};
			let tmpAction = tmpBody.Action;
			let tmpArgs;
			let tmpLabel;
			if (tmpAction === 'clean')
			{
				tmpArgs  = ['cache', 'clean', '--force'];
				tmpLabel = 'npm cache clean --force';
			}
			else if (tmpAction === 'verify')
			{
				tmpArgs  = ['cache', 'verify'];
				tmpLabel = 'npm cache verify';
			}
			else
			{
				respondError(pRes, 400, 'BadAction',
					'Action must be "clean" or "verify".');
				return pNext();
			}
			...
			pRes.statusCode = 202;
			pRes.send({ OperationId: tmpOperationId, Action: tmpAction });
			return pNext();
		});
